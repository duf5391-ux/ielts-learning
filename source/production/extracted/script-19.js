(() => {
  ...
  function freeze(unit) {
    ...
    updateFrozen(unit);
    wordCounts(unit);
    notice('首稿已保留；下方修订稿可继续修改，首稿保持原样。');
  ...
  function onHash() {
    ...
    } else if (hash === 'writing-workbench') selectUnit(picker.value);
    if (!root.hidden) {
  ...
  function exportUnit(unit) {
    const rows = [`IELTS 写作工作台 · ${unit.querySelector('h2').textContent}`, `导出时间：${new Date().toLocaleString()}`, '', '【题目与来源】', unit.querySelector('.ww-prompt').textContent, unit.querySelector('.ww-backlinks').previousElementSibling.textContent, ''];
    ...
    rows.push('【可发给助手或老师的反馈请求】', '请结合题目、首稿、修订稿和作答条件，按 TA/TR、CC、LR、GRA 检查。每项给出具体作品证据，只优先选 1–2 个问题，解释修改理由，再安排一个局部练习。不要仅凭字数、复杂词数量或勾选结果给分。');
