...
function fixture({initial=null,allStorage=null,quota=false,hash='#study'}={}){
 ...
 const importer=document.querySelector('#import-state'); const oldAdd=importer.addEventListener.bind(importer); importer.addEventListener=(type,fn)=>{if(type==='change'){(importer.syntheticChanges??=[]).push(fn);importer.syntheticChange=async e=>{for(const handler of importer.syntheticChanges)await handler(e)}}else oldAdd(type,fn)};
...
(async()=>{await test('Three stores export/import restore into fresh profile',async()=>{const f=boot();f.input('ww-primary-first','QA original text');f.click(f.q('#records [data-export="json"]'));const raw=await f.downloads.at(-1).text();const g=boot();await g.q('#import-state').syntheticChange({target:{files:[{name:'qa.json',size:Buffer.byteLength(raw),text:async()=>raw}],value:''}});console.log(JSON.stringify({reloads:g.reloads,raw:g.storage,status:g.q('#save-status').textContent}));assert.equal(JSON.parse(g.storage).fields['ww-primary-first'],'QA original text');});console.log(checks);})();