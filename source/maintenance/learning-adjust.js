(function () {
  ...
  function save(next) {
    if (blocked) { notify('安排记录无法读取，原文已保留。请先到工作区导出记录。'); return false; }
  ...
  function unitProgress(u) { return M.progress(u.steps.map(s => s.kind === 'checked' ? !!state.checked[s.key] : s.kind === 'check' ? value(s.key) === true : M.filled(value(s.key)))); }
  ...
  function renderParts() {
    for (const n of $$('[data-unit-progress]')) { const u=units.get(n.dataset.unitProgress); if(u)n.replaceChildren(bar(unitProgress(u),u.progressLabel||'完成')); }
    for (const n of $$('[data-collection-progress]')) { const ids=JSON.parse(n.dataset.collectionProgress).filter(id=>units.has(id));n.replaceChildren(bar(M.progress(ids.map(id=>unitProgress(units.get(id)).percent===100)),'单元')); }
  }
  function gateValues(keys) {return Object.fromEntries(keys.map(k=>[k,String(value(k)||'')]))}
  function canOpen(gate) { const keys=JSON.parse(gate.dataset.answerGate); return M.canCheck(keys,gateValues(keys)); }
  function reveal(gate, requested) {
    if (!canOpen(gate)) {gate.open=false;gate.querySelector('.la-gate-content').hidden=true;if(requested){notify('先留下这组题的答案，再核对。不会的题可以写“暂时不会”。');const key=JSON.parse(gate.dataset.answerGate).find(k=>!M.filled(String(value(k)||'')));fields.get(key)?.focus();}return false;}
    if(requested && !state.checked[gate.id]) {const keys=JSON.parse(gate.dataset.answerGate);if(!save(M.check(state,gate.id,keys,gateValues(keys),new Date().toISOString()))){gate.open=false;return false;}}
    gate.querySelector('.la-gate-content').hidden=false;return true;
  }
  for(const gate of $$('[data-answer-gate]')) {
  ...
  function refreshGates(){for(const gate of $$('[data-answer-gate]'))gate.classList.toggle('la-gate-locked',!canOpen(gate));}
  ...
  function panelRoute(){let h;try{h=decodeURIComponent(location.hash.slice(1))||(data.navigationVersion>=3?'study':'guide');}catch{return;}const n=document.getElementById(h),panel=n?.closest('main > .panel');if(!panel)return;
    ...
    const owner=data.navigationVersion&&u?u.mode:panel.dataset.laOwner||panel.id;for(const b of $$('#workspace-navigation [data-go]')){if(b.dataset.go===owner)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current');}
    const sectionName={guide:'今天',study:'学习',practice:'练习',tests:'测试',workspace:'工作区'}[owner]||'学习';
  ...
  document.addEventListener('input',e=>{const target=e.target;if(!target?.dataset.save||drawing)return;requestAnimationFrame(()=>{drawing=true;try{renderParts();refreshGates();if(['#guide','#plan','#study'].includes(location.hash))renderToday();if(/^#(study|practice)(-|$)/.test(location.hash))renderCards();if(target.dataset.testAnswer){const id=target.dataset.testAnswer;$('#test-'+id+' [data-test-progress]').replaceChildren(bar(M.progress(Object.values(answerValues(id)).map(M.filled)),'已作答'));const part=target.closest('[data-test-part]');if(part)part.querySelector('[data-test-part-progress]').replaceChildren(bar(M.progress([...part.querySelectorAll('[data-test-answer]')].map(f=>M.filled(f.value))),'已作答'));}}finally{drawing=false;}});});
  ...
  document.addEventListener('ielts-record-committed',()=>{renderParts();refreshGates();renderToday();renderCards();});
  renderCards();renderToday();renderParts();refreshGates();renderTests();panelRoute();setInterval(renderClocks,1000);
