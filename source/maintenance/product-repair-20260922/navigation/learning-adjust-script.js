(function () {
  ...
  document.addEventListener('click',event=>{
    ...
    for(const mode of ['study','practice'])if(!$('#'+mode).hidden){rememberFilters(mode);listPositions.set(mode+':'+scope[mode],window.scrollY);}
  },true);
  function unitCard(u) {
  ...
  function panelRoute(){let h;try{h=decodeURIComponent(location.hash.slice(1))||(data.navigationVersion>=3?'study':'guide');}catch{return;}const n=document.getElementById(h),panel=n?.closest('main > .panel');if(!panel)return;
    ...
    if(panel.id==='study'||panel.id==='practice'){const y=listPositions.get(panel.id+':'+scope[panel.id]);if(y!==undefined)requestAnimationFrame(()=>requestAnimationFrame(()=>window.scrollTo(0,y)));}
