(function () {
  ...
  function unitCard(u) {
    ...
    if(u.referenceConfidence){const metadata=el('p','','content-entry-metadata');metadata.textContent='难度 '+(u.difficulty?.level?u.difficulty.level+'/5':'待评估')+' · 参考置信度'+u.referenceConfidence+' · '+u.source;metadata.title=(u.difficulty?.reason||u.difficulty?.rationale||'')+'；参考置信度是资料使用策略，不是命中率。';n.append(metadata);}
    const actions=el('div','','la-card-actions');actions.append(link(u.id,u.mode==='practice'?'开始练习 →':'进入学习 →'),button(state.today.includes(u.id)?'移出自选':'加入自选',()=>toggleToday(u.id)));n.append(actions);return n;
  }
  ...
  function panelRoute(){let h;try{h=decodeURIComponent(location.hash.slice(1))||(data.navigationVersion>=3?'study':'guide');}catch{return;}const n=document.getElementById(h),panel=n?.closest('main > .panel');if(!panel)return;
    ...
    if(data.navigationVersion){
      ...
      if(category){lastCategory={mode:category[1],skill:category[2]};scope[category[1]]=category[2];for(const suffix of ['skill','topic','search','type','part']){const f=$('#la-'+category[1]+'-'+suffix);if(f)f.value='';}
    ...
    const owner=panel.dataset.laCollectionTitle?'practice':data.navigationVersion&&u?u.mode:panel.dataset.laOwner||panel.id;document.body.classList.toggle('ws-wide',owner==='workspace'||owner==='development');for(const b of $$('#workspace-navigation [data-go]')){if(b.dataset.go===owner)b.setAttribute('aria-current','page');else b.removeAttribute('aria-current');}
    const sectionName={'word-review':'复习','vocabulary-review':'单词','sentence-learning':'句子',guide:'今天',study:'学习',practice:'练习',tests:'测试',workspace:'工作台',development:'开发工作台'}[owner]||'学习';
    $('#current-page-label').textContent=u?sectionName+' / '+u.skillLabel+' / '+u.title:((panel.id==='study'||panel.id==='practice')&&scope[panel.id]?sectionName+' / '+(groups[scope[panel.id]]||data.skills[scope[panel.id]]):panel.dataset.laCollectionTitle||panel.dataset.laTitle||sectionName);
