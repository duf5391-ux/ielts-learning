(() => {
  ...
  function save(rows, options = {}) {
    ...
    if (options.requirePersistence) {
      lastPersisted = window.IELTSRecordStore?.commit({[field.dataset.save]:next}) === true;
    } else {
  ...
  function setFavorite(term, value = true) {
    return updateEntry(term, row => ({favorite:!!value, favoriteAt:value ? (row.favoriteAt || new Date().toISOString()) : row.favoriteAt}));
  }
