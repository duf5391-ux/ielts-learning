(() => {
  ...
  const PHASE_NAMES = ['先试一次', '理解与辨析', '带支架练习', '独立迁移', '隔日再用'];
  ...
  function validateCoursePackage(input) {
    ...
    const courses = list(input.courses, '课程', 50, 1).map((c, ci) => {
      ...
      result.stages = list(c.stages, label + '学习阶段', 5, 5).map((s, si) => {
  ...
  const announce = (text, error = false) => { message.textContent = text; message.classList.toggle('cw-error', error); message.setAttribute('role', error ? 'alert' : 'status'); };
  ...
  introduction.append(el('p', '你给资料，我设计贴合考试的讲解与练习', 'cw-flow'), el('p', '双向拟合 = 你的材料 ↔ 雅思考试。我会按相关真题、官方样例和有评分依据的作答选材，说明这批资料适合怎样用于目标题型，再从考试任务反查材料和练习是否贴合。重点是来源与设计的考试拟合度，不以一批资料保证覆盖整个考试。', 'cw-muted'), el('p', '可以直接在当前对话发材料，我设计后放回这里。这个窗口按阶段学习；作答记录用于另外调整支架和下一步。下方保存、导出和导入是可选的整理与备份工具。', 'cw-muted'));
  ...
  mount.replaceChildren(introduction, message, toolbar, intake, listRegion, workspace);
  ...
  form.addEventListener('submit', event => { event.preventDefault(); safe(() => {
    ...
    update(s => { const index = s.drafts.findIndex(d => d.id === draft.id); if (index < 0) s.drafts.push(draft); else s.drafts[index] = draft; s.selected = draft.id; }, '已保存为待设计。导出需求后，我会基于原料、目标和学习记录设计课程。');
  ...
  function choose(value) {
    ...
    safe(() => update(s => { s.selected = selected; })); renderList(); renderWorkspace();
  }
  function renderList() {
    listRegion.replaceChildren(); const courses = allCourses(), userCourses = courses.filter(c => c.status !== 'sample');
    listRegion.append(el('h2', '我的学习窗口'), el('p', state.drafts.length + ' 批待设计 · ' + userCourses.length + ' 门已设计课程 · 示例单独体验，不计入我的批次。', 'cw-muted'));
    const choices = el('div', '', 'cw-course-choices');
    for (const d of [...state.drafts].reverse()) { const b = button(d.title + ' · 待设计', () => choose(d.id)); b.dataset.cwCourse = d.id; b.setAttribute('aria-pressed', String(selected === d.id)); choices.append(b); }
  ...
  function renderWorkspace() {
    ...
    const budgetControl = el('div', '', 'cw-budget'); budgetControl.append(el('span', '今天可用时间'));
    ...
    const workload = budget === 5 ? '先做本阶段 1 道任务，保留一次真实作答。' : budget === 10 ? '建议做本阶段前 2 道任务，留时间对照并自评。' : '可完成当前阶段的任务，再选择是否进入下一阶段。';
    header.append(budgetControl, el('p', workload + ' 今天少做不会形成欠账；阶段可以跳转。', 'cw-muted'));
    const action = nextAction(c), recommendation = el('aside', '', 'cw-next'); recommendation.append(el('strong', '根据最近一次证据'), el('p', action.text));
    ...
    recommendation.append(button('进入建议阶段 →', () => goStage(c, action.stage, nextTask)));
    ...
    workspace.append(header, recommendation);
    const stages = el('nav', '', 'cw-stages'); stages.setAttribute('aria-label', '学习阶段');
    ...
    const stage = c.stages.find(s => s.id === liveStage), stageArea = el('section', '', 'cw-stage-content'); stageArea.append(el('p', '当前阶段 ' + (PHASES.indexOf(stage.id) + 1) + ' / 5 · 设计用时约 ' + stage.minutes + ' 分钟', 'cw-kicker'), el('h3', stage.title), el('p', stage.purpose));
    ...
    const previous = button('← 上一阶段', () => goStage(c, PHASES[index - 1])); previous.disabled = index === 0;
    const next = button('下一阶段／先跳过 →', () => goStage(c, PHASES[index + 1])); next.disabled = index === 4;
  ...
  function renderTask(c, stage, t, index) {
    ...
    if (submitted) {
      ...
      else card.append(el('p', '文字作答不自动判分。请对照下方参考与标准，给这次表现一个自评。', 'cw-muted'));
      appendReference(card, t);
      const assessment = el('div', '', 'cw-assessment'); assessment.append(el('span', '这次表现：'));
    ...
    const reveal = button('先看参考（会记录看过答案）', () => { recordExposure(c, t, 'reference'); if (!card.querySelector('.cw-reference')) appendReference(card, t); reveal.disabled = true; }); reveal.dataset.cwReveal = t.id; reveal.disabled = draft.revealed;
    actions.append(submit, reveal); card.append(actions);
    if (draft.revealed) appendReference(card, t);
    if (draft.sourceSeen || draft.revealed) card.append(el('p', '已记录本次看过讲解／参考，不能标记成无提示表现。', 'cw-muted'));
    return card;
  }
  function renderFitMap(c) {
    const details = el('details', '', 'cw-fit'); details.id = 'cw-fit-map'; details.append(el('summary', '双向拟合表：材料 ↔ 雅思考试（含原料提示）'));
