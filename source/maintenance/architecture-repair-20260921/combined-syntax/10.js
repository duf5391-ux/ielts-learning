(() => {
  ...
  const title=el('div'); title.append(el('p','MY VOCABULARY','vr-kicker'),el('h1','我的单词表'),el('p','把查过的词与词组收藏起来，连同原句和搭配一起复习。','vr-muted'));
  heading.append(title); host.append(heading);
  const stats=el('div','','vr-stats'); stats.id='vr-stats'; host.append(stats);
  ...
  controls.append(mode,start,extra); host.append(controls);
  const status=el('p','','vr-status'); status.id='vr-status'; status.setAttribute('role','status'); host.append(status);
  const stage=el('div','','vr-stage'); stage.id='vr-stage'; stage.hidden=true; host.append(stage);
  ...
  function renderList() {
    ...
    if (!rows.length) { list.append(el('p',favorites().length?'没有符合条件的词语。':'查词后点“收藏到单词表”，或者选中正文词组后收藏。','vr-empty')); return; }
    let shown=0;
  ...
  function refresh() { renderStats(); renderList(); renderLegacy(); }
  function begin(scope, row) {
    ...
    session={keys:rows.map(key),index:0,done:0,mode:mode.value,scope,practice:scope==='all'||scope==='one'&&!due(row)}; stage.hidden=false; library.open=false; nextCard();
  }
  function nextCard() {
    ...
    if(!current){stage.replaceChildren(el('h3',session.practice?'这一轮加练完成':'这一轮复习完成'),el('p',`已完成 ${session.done} 个词。`+(session.practice?'加练答案已记录，下次复习时间保持不变。':'每个词的下次时间已安排好。')),button('返回单词表',()=>{stage.hidden=true;library.open=true;session=null;refresh();}));announce(`本轮完成 ${session.done} 个词。`);return;}
