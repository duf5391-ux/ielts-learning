(() => {
  ...
  function begin(scope, row) {
    ...
    session={keys:rows.map(key),index:0,done:0,mode:mode.value,scope,practice:scope==='all'||scope==='one'&&!due(row)};host.dataset.vrSession='true'; stage.hidden=false; library.open=false; nextCard();
  }
  function nextCard() {
