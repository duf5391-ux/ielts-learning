...
(async()=>{
 ...
 await check('new Task 2 completes outline→first→freeze→four dimensions→revision versions→export and reload',async p=>{
  ...
  const downloaded=p.waitForEvent('download');await p.locator('#'+id+' [data-ww-action=export]').click();const d=await downloaded;await d.saveAs(path.join(__dirname,'export-new-task.txt'));
  const output=fs.readFileSync(path.join(__dirname,'export-new-task.txt'),'utf8');assert(output.includes(text)&&output.includes('成人读写困难')&&output.includes('九分学长'));
 ...
 await check('mobile choosing a task opens first draft within one screen; prompt, stages and resume remain usable',async p=>{
  assert(await p.locator('.ww-directory').isVisible());assert.equal(await p.locator('.ww-directory-row:visible').count(),32);await p.screenshot({path:path.join(__dirname,'mobile-directory.png')});
  ...
  await field(p,id+'-first').fill('Mobile first draft survives returning.');await p.screenshot({path:path.join(__dirname,'mobile-draft.png')});
 ...
 await check('desktop flat catalogue lists truthful metadata and search reaches a case draft',async p=>{
  ...
  await p.locator('.ww-directory-row:visible').click();await p.locator('#ww-wc-c21-t3-rainshadow-v1-draft').waitFor({state:'visible'});await p.screenshot({path:path.join(__dirname,'desktop-draft.png')});
 });
 ...
 await browser.close();server.close();const report={candidate,candidateSha256:require('crypto').createHash('sha256').update(bytes).digest('hex'),pass:checks.every(c=>c.pass)&&!errors.length,checks,errors};fs.writeFileSync(path.join(__dirname,'qa-results.json'),JSON.stringify(report,null,2));console.log(JSON.stringify(report,null,2));
 }
