(() => {
  ...
  host.replaceChildren();const links=el('nav','','word-tool-links');for(const [id,label] of [['word-review','开始复习'],['topical-vocabulary','话题词卡'],['lookup-learning','查词历史']]){const a=el('a',label);a.href='#'+id;links.append(a);}host.append(links);
  ...
  function renderStats() {
    ...
    summary.textContent=`查看单词表 · ${rows.length} 个词与词组`;title.querySelector("h1").textContent="我的单词 · "+rows.length;
  }
  function renderList() {
